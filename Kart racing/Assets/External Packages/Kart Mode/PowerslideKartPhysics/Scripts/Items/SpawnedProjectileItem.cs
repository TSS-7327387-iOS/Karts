            ...
            for (int i = 0; i < contactCount; i++)
            {
                ...
                if (curCol.otherCollider.IsKart())
                {
                    if (curCol.otherCollider != casterCol || (lifeTime > casterIgnoreTime && canHitCaster && curCol.otherCollider == casterCol))
                    {
                        ...
                        if (!curCol.otherCollider.transform.GetTopmostParentComponent<Kart>().isShieldActivated)
                        {
                            
                            curCol.otherCollider.transform.GetTopmostParentComponent<Kart>().SpinOut(kartSpin, kartSpinCount);
                            Destroy(gameObject);
                            if (curCol.otherCollider.transform.GetComponentInParent<Kart>() != null)//Decrease health bar in player UI
                            {
                                curCol.otherCollider.transform.GetComponentInParent<Kart>().takeDamage_HealthSlider(0.2f);
                            }
                        }
                      /*  curCol.otherCollider.transform.GetTopmostParentComponent<Kart>().SpinOut(kartSpin, kartSpinCount);
                        Destroy(gameObject);*/
